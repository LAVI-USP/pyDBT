...
%{
% -------------------------------------------------------------------------
%                 projectionDDb_cuda_lib(pVolume, pProj, ppGeo)
% -------------------------------------------------------------------------
%     DESCRIPTION:
	...
	const double DAG)
{
	...
	printf("GPU Device %d: \"%s\" with compute capability %d.%d has %d Multi-Processors and %zu bytes of global memory\n\n", devID,
	...
	*/
	...
	map_boudaries_kernel << <blockSize, threadsPerBlock >> > (d_pObjX, nPixXMap, (double)nPixX, -dx, x_offset);

	blockSize.x = (nPixY / maxThreadsPerBlock) + 1;

	map_boudaries_kernel << <blockSize, threadsPerBlock >> > (d_pObjY, nPixYMap, nPixY / 2.0, dy, y_offset);
	...
	//printf("Map Boundaries -- Threads:%d Blocks:%d \n", threads, blocks);
	...
	for (int k = 0; k < ceil((double)nPixXMap / (threadsPerBlock.x-1)); k++) {
		...
		//printf("integration kernel -- threadsX:%d blocksX:%d threadsY:%d blocksY:%d \n", threadsPerBlock.x, blockSize.x, threadsPerBlock.y, blockSize.y);
	...
	for (unsigned int p = projIni; p < projEnd; p++) {
		...
		for (unsigned int nz = 0; nz < nSlices; nz++) {
			...
			*/
			...
			//printf("Map detector onto XY plane -- ThreadsX:%d ThreadsY:%d BlocksX:%d BlocksY:%d\n", threadsPerBlock.x, threadsPerBlock.y, blockSizeX, blockSizeY);
